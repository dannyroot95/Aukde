...
class GeofireStoreProvider(reference: String , provider : String) {

    private var mDatabase: DatabaseReference = FirebaseDatabase.getInstance().reference.child(reference).child(provider)
    ...
    fun getActiveStore(latLng: LatLng, radius: Double): GeoQuery {
    ...
    fun getDriverKeyLocation(idDriver: String?): DatabaseReference {
        return mDatabase.child(idDriver!!).child("g")
    }
